...
export class PayPalProvider implements PaymentProvider {
  ...
  private parseWebhookEvent(event: any): WebhookEvent {
    ...
    if (eventType === 'PAYMENT.CAPTURE.COMPLETED') {
      const orderId = resource.custom_id || resource.purchase_units?.[0]?.reference_id || ''
  ...
  async processPayment(event: WebhookEvent): Promise<PaymentResult> {
    if (!event.orderId || !event.transactionId) {
      throw new Error('Missing orderId or transactionId in PayPal webhook event')
    }
    ...
    return {
      ...
      message: event.type === 'payment.success'
