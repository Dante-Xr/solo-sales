...
/** 当前前台唯一可用结账渠道：Stripe 托管信用卡支付。 */
const paymentMethods = ["Credit / Debit Card", "Stripe"]
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-12">
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              <div className="grid size-10 place-items-center rounded-lg bg-foreground">
                <span className="font-bold text-background text-lg">S</span>
              </div>
              <span className="text-xl font-bold">Solo Sales</span>
            </div>
          ...
          <div className="hidden md:block">
            <div className="grid grid-cols-2 gap-6 md:gap-8">
              <div>
                <h3 className="font-semibold mb-4 text-sm md:text-base">{t("shop")}</h3>
                <ul className="space-y-2">
                  {footerLinks.shop.map((link) => (
                    <li key={link.href}>
                      <Link
              ...
              <div>
                <h3 className="font-semibold mb-4 text-sm md:text-base">{t("company")}</h3>
                <ul className="space-y-2">
                  {footerLinks.company.map((link) => (
                    <li key={link.href}>
                      <Link
          ...
          <div className="md:col-span-2 lg:col-span-1">
            <h3 className="font-semibold mb-4 text-sm md:text-base">{t("newsletter")}</h3>
            ...
            <form onSubmit={handleSubscribe} className="flex gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  className="pl-10 border-border bg-background text-foreground placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/30"
                />
              </div>
              <Button
                type="submit"
          ...
          <div className="flex items-center justify-center gap-3 mb-4">
            {paymentMethods.map((method, index) => (
              <span key={method} className="flex items-center text-muted-foreground text-xs">
                {index > 0 && <span className="mr-3 text-border">|</span>}
                <span className="px-2 py-1 bg-muted rounded text-[11px] font-medium">{method}</span>
              </span>
            ))}
          </div>
