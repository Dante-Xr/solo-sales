...
export default function AdminProfilePage() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || t('failedToLoadProfile'))
      }
    } catch (err) {
  ...
  const handlePasswordSave = async () => {
    ...
    if (!passwordData.oldPassword) {
      setPasswordError(t('pleaseEnterOldPassword'))
      return
    }

    if (!passwordData.newPassword) {
      setPasswordError(t('pleaseEnterNewPassword'))
      return
    }

    if (passwordData.newPassword.length < 6) {
      setPasswordError(t('newPasswordMustBeAtLeast6Characters'))
      return
    }
