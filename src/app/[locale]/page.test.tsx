...
jest.mock("@/server/services/product-service", () => ({
  ...
  getStorefrontCategories: jest.fn(),
}))

jest.mock("@/components/storefront/EnhancedProductShowcase", () => ({
  EnhancedProductShowcase: ({ products, categories }: { products: unknown[]; categories: unknown[] }) => (
    <div data-testid="product-showcase">
      Showcase with {products.length} products and {categories.length} categories
    </div>
  ),
}))
...
jest.mock("@/components/storefront/HeroBanner", () => ({
  HeroBanner: () => <section data-testid="hero">Hero</section>,
}))
...
import { getFeaturedProducts, getStorefrontCategories } from "@/server/services/product-service"

const getFeaturedProductsMock = getFeaturedProducts as jest.MockedFunction<typeof getFeaturedProducts>
const getStorefrontCategoriesMock = getStorefrontCategories as jest.MockedFunction<typeof getStorefrontCategories>
...
describe("Storefront (HomePage)", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    getStorefrontCategoriesMock.mockResolvedValue([
  ...
  it("应该从商品服务获取商品并渲染", async () => {
    ...
    expect(screen.getByTestId("product-showcase")).toHaveTextContent("2 products and 2 categories")
  })
  ...
  it("应该使用商品服务返回的缓存数据", async () => {
    ...
    expect(screen.getByTestId("product-showcase")).toHaveTextContent("1 products")
  })
  ...
  it("应该在商品服务失败时渲染兜底商品", async () => {
    ...
    try {
      ...
      expect(screen.getByTestId("product-showcase")).toHaveTextContent("6 products")
    } finally {
