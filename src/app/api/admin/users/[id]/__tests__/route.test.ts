...
describe("/api/admin/users/[id]", () => {
  ...
  it("rejects unauthenticated user detail access", async () => {
    const response = await GET(
      { headers: new Headers() } as never,
      { params: Promise.resolve({ id: "user_1" }) } as never
    )
  ...
  it("rejects unauthenticated user updates", async () => {
    const request = {
      headers: new Headers(),
      json: async () => ({ name: "Updated Name" }),
    } as never

    const response = await PATCH(request, { params: Promise.resolve({ id: "user_1" }) } as never)
    const body = await response.json()
  ...
  it("rejects unauthenticated user deletion", async () => {
