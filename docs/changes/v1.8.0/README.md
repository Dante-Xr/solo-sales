...
- Vercel 受保护的 Scheduled Route Handler 由外部定时器每分钟触发，并配套后台“任务调度”控制面：权限、启停、间隔、批量、运行历史、死信和手动执行。

## 发布边界

代码、迁移与自动化验证已完成；当前生产目标为 Vercel，仍须完成外部定时器、真实 Redis、SMTP、OTP 重置和故障恢复验收。验收通过后更新发布证据并推送 v1.8.0 tag。
